/**
 * woodmill-extraction-alert — timer-triggered poller (every 15 min).
 *
 * Polls woodmill_extraction_checks for rows that have a flagged item
 * (any_flag = true) and have not yet been alerted (alerted_at is null),
 * emails QHSE + the wood-mill area owner via Graph (from systemapp@, the
...
// QHSE gets the alert; the wood-mill area owner (per the Near Miss owner map) is CC'd.
...
module.exports = async function (context) {
  const rows = await sbGet(
    'woodmill_extraction_checks?any_flag=eq.true&alerted_at=is.null' +
    '&select=id,operator_name,submitted_at,flag_count,results&order=submitted_at.asc',
  );
  if (!rows.length) { context.log('woodmill-extraction-alert: nothing to alert.'); return; }

  const t = await token();
  for (const check of rows) {
    const { subject, html } = buildAlertEmail(check);
    await sendMailCc(t, ALERT_TO, ALERT_CC, subject, html);
    await sbPatch(`woodmill_extraction_checks?id=eq.${check.id}`, { alerted_at: new Date().toISOString() });
    context.log(`woodmill-extraction-alert: emailed QHSE for check ${check.id} (${check.flag_count} flagged).`);
  }
};
