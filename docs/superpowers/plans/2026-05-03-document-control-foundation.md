...
let _qmsDriveIdCache = null;
async function _getQmsDriveId() {
  if (_qmsDriveIdCache) return _qmsDriveIdCache;
  const siteId = await getQmsSiteId();
  // List all drives on the Quality site and find the QMS-Documents library by name
  const drivesResp = await graphGet(`https://graph.microsoft.com/v1.0/sites/${siteId}/drives`);
  const drives = drivesResp.value || [];
  const qms = drives.find(d => d.name === QMS_DOC_LIB_NAME);
  if (!qms) throw new Error(`Document library "${QMS_DOC_LIB_NAME}" not found on the Quality site. Available drives: ${drives.map(d => d.name).join(', ')}`);
  _qmsDriveIdCache = qms.id;
  return _qmsDriveIdCache;
}

async function uploadDocFile(category, fileName, fileBlob) {
  // category = 'HS' | 'Quality' | 'Group'
  const driveId = await _getQmsDriveId();
  const token = await getGraphToken();
...
.docs-side { background: #fff; border: 1px solid var(--border); border-radius: 16px; padding: 18px; position: sticky; top: 20px; max-height: calc(100vh - 40px); overflow-y: auto; }
